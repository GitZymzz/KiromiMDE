...
async function scrapeStoryAnime() {
    try {
        let randomUrl = videoUrls[Math.floor(Math.random() * videoUrls.length)];
        let response = await axios.head(randomUrl); // Cek apakah file masih ada
...
module.exports = { scrapeStoryAnime }; 